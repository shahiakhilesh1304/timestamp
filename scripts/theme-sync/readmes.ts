...
export function generateMasterReadme(): string {
  ...
  const tableRows = sortedThemes
    .map((theme) => {
      ...
      const worldMap = theme.supportsWorldMap ? '✅' : '❌';
      const author = theme.author
        ? `[@${theme.author}](https://github.com/${theme.author})`
        : 'Anonymous';

      const localLink = `[🏠](${SITE_URL}?mode=wall-clock&target=${newYear}&theme=${theme.id})`;
      const globalLink = `[🌐](${SITE_URL}?mode=absolute&target=${productLaunch.date}&theme=${theme.id}&message=Launch%20Day!)`;
      const timerLink = `[⏱️](${SITE_URL}?mode=timer&duration=300&theme=${theme.id})`;
...
_Total: ${themes.length} themes_ · Demo links: 🏠 Local Time · 🌐 Same Moment · ⏱️ Timer
...
function writeMasterReadme(): void {
  ...
  console.log('✅ Generated src/themes/README.md');
}
...
function writeThemeReadmes(): void {
  ...
  for (const themeId of themeIds) {
    ...
    writeFileSync(filePath, content, 'utf-8');
    console.log(`✅ Generated src/themes/${themeId}/README.md`);
  }
}
...
function checkReadmeSync(): boolean {
  ...
  for (const themeId of themeIds) {
    ...
    if (!existsSync(themePath)) {
      console.error(`❌ Missing: src/themes/${themeId}/README.md`);
      allMatch = false;
    } else {
      ...
      if (expectedTheme !== actualTheme) {
        console.error(`❌ Out of sync: src/themes/${themeId}/README.md`);
        allMatch = false;
      }
    }
  }
...
export function syncReadmes(): boolean {
  try {
    ...
    console.log(`📋 Found ${themeCount} themes in THEME_REGISTRY`);
    writeMasterReadme();
    writeThemeReadmes();
    console.log(`\n✅ Generated ${themeCount + 1} README files`);
    return true;
  } catch (error) {
    console.error('❌ Failed to sync READMEs:', error);
    return false;
  }
}
...
export function checkReadmesSync(): boolean {
  ...
  if (inSync) {
    console.log('✅ All theme READMEs are in sync with THEME_REGISTRY');
    return true;
  }

  console.error('❌ Theme READMEs are out of sync with THEME_REGISTRY');
